...
- `deployments/` - alamat router mainnet yang dipakai runtime.
- `artifacts/` - compiled EVM router artifact.
- `.env` - local signer/env, di-ignore git.

## Native Bridge

Runtime supports native ETH bridge quotes/execution from Ethereum and Base to Arc Mainnet through `ArcoxNativeSwapBridgeRouter`.

Supported examples:

```bash
bridge 0.001 ETH from Base to Arc
bridge 0.001 ETH from Ethereum to Arc
```
